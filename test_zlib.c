...
#define INBUFSIZ   1026         /* 入力バッファサイズ（任意） */
#define OUTBUFSIZ  1026         /* 出力バッファサイズ（任意） */
...
void do_compress(void)          /* 圧縮 */
{
  ...
  /* 基本的な仕様として、z.next_in が0になるとz.next_in, z.avail_inを更新しなければならない*/ 
  while (1) {
    if (z.avail_in == 0) {  /* 入力が尽きれば */
      ...
      if (z.avail_in < INBUFSIZ) flush = Z_FINISH; /*ここの終了処理は変更する必要がありそう*/
    }
...
{
  int count, status;
  Bytef *dict = (char *)malloc(32769);//辞書は最大 32768 byte
  ...
  while (status != Z_STREAM_END) {
    ...
    //**************************************************************************
    if(inflateGetDictionary(&z,dict,dict_size)==Z_OK){
      printf("%s\n",dict);
      printf("%u\n",result);
    }
    //**************************************************************************
